...
(async () => {
  ...
  for (const f of ['test/00_baseline_guess.sql', '01_additive.sql', '02_lock.sql', '04_challenges.sql', '05_rooms.sql', '05_rooms.sql']) await file(f);
  ...
  const tok = {}, id = {};
  for (const [n, k, ts] of [['Айгүл', '3А', [2, 5, 10, 3]], ['Ерасыл', '3А', [2, 5, 10]], ['Дана Қасымова', '3А', []], ['Бөтен', '4Б', [2]], ['Санжар', '3А', [5]], ['Мадина', '3А', [5]]]) {
  ...
  const right = items => items.map(([a, b]) => a * b);

  // ── opening a room
  T('create: a table I have not passed / nonsense / a mix with fewer than two tables', (await rpc('esep_room_create', { p_token: tok['Ерасыл'], p_table: 3 })).error === 'not_passed'
    && (await rpc('esep_room_create', { p_token: tok['Ерасыл'], p_table: 11 })).error === 'table' && (await rpc('esep_room_create', { p_token: tok['Санжар'], p_table: 0 })).error === 'not_passed'
    && (await rpc('esep_room_create', { p_token: tok['Дана Қасымова'], p_table: 2 })).error === 'not_passed' && (await rpc('esep_room_create', { p_token: 'x'.repeat(48), p_table: 2 })).error === 'session');
  const R = await rpc('esep_room_create', { p_token: tok['Айгүл'], p_table: 5 });
  T('create: a four-digit code; the host is the first player', /^\d{4}$/.test(R.code) && R.id > 0, R);
  T('create again while it is live: the same room comes back, no second one', (await rpc('esep_room_create', { p_token: tok['Айгүл'], p_table: 2 })).id === R.id);
  let s = await rpc('esep_room_state', { p_token: tok['Айгүл'], p_id: R.id });
  T('state (lobby): host flag, one player, NO facts yet', s.status === 'lobby' && s.is_host && s.n === 1 && JSON.stringify(s.players) === '["Айгүл"]' && !('items' in s), s);
  T('start alone is refused', (await rpc('esep_room_start', { p_token: tok['Айгүл'], p_id: R.id })).error === 'alone');

  // ── finding it, joining it
  let l = await rpc('esep_room_list', { p_token: tok['Ерасыл'] });
  T('list: a classmate sees the open room (no code needed); the host sees it as «mine»; another class sees nothing', l.open.length === 1 && l.open[0].host_name === 'Айгүл' && l.open[0].table === 5
  ...
  T('join by code: a classmate; another class too (whole school); twice is harmless', (await rpc('esep_room_join', { p_token: tok['Ерасыл'], p_code: R.code })).id === R.id
    && (await rpc('esep_room_join', { p_token: tok['Бөтен'], p_code: R.code })).id === R.id && (await rpc('esep_room_join', { p_token: tok['Ерасыл'], p_code: ' ' + R.code + ' ' })).id === R.id);
  const jd = await rpc('esep_room_join', { p_token: tok['Дана Қасымова'], p_code: R.code });
  T('join: a pupil who has not passed the table is let in, and told so', jd.id === R.id && jd.not_passed === true && (await rpc('esep_room_join', { p_token: tok['Санжар'], p_code: R.code })).not_passed === false, jd);
  T('an outsider cannot look into the room', (await rpc('esep_room_state', { p_token: tok['Мадина'], p_id: R.id })).error === 'not_found');
  T('a host cannot wander into another room', (await rpc('esep_room_create', { p_token: tok['Мадина'], p_table: 5 })).id > 0 && (await rpc('esep_room_join', { p_token: tok['Мадина'], p_code: R.code })).error === 'hosting');
  ...
  T('nothing can be handed in before the start', (await rpc('esep_room_submit', { p_token: tok['Ерасыл'], p_id: R.id, p_answers: [1], p_ms: 1 })).error === 'not_started');
  ...
  T('state (running): the same ten facts of the 5 table for everybody, a countdown, a clock', s.status === 'running' && s.items.length === 10 && s.items.every(([a, b]) => a === 5 || b === 5)
    && JSON.stringify(s.items) === JSON.stringify((await rpc('esep_room_state', { p_token: tok['Дана Қасымова'], p_id: R.id })).items) && s.starts_in > 0 && s.starts_in <= 4000 && s.left > 170000, s);
  T('nobody comes in after the start', (await rpc('esep_room_join', { p_token: tok['Мадина'], p_code: R.code })).error === 'started');
  const items = s.items, ok = right(items);
  T('nothing can be handed in during the countdown either', (await rpc('esep_room_submit', { p_token: tok['Ерасыл'], p_id: R.id, p_answers: ok, p_ms: 1 })).error === 'not_started');
  ...
  await at(20); const a2 = await rpc('esep_room_submit', { p_token: tok['Айгүл'], p_id: R.id, p_answers: ok, p_ms: 5 });
  T('submit: marked on the server (10/10); the time is the SERVER\'s — a browser claiming 5 ms gets the 20 s that really passed', a2.ok === true && a2.my_ok === 10 && a2.my_ms >= 20000 && a2.my_ms < 23000, a2);
  await at(40); const a1 = await rpc('esep_room_submit', { p_token: tok['Ерасыл'], p_id: R.id, p_answers: ok, p_ms: 1 });
  T('submit twice: the first result stands', a1.my_ms >= 40000 && (await rpc('esep_room_submit', { p_token: tok['Ерасыл'], p_id: R.id, p_answers: [], p_ms: 1 })).my_ok === 10, a1);
  T('junk answers count for nothing', (await rpc('esep_room_submit', { p_token: tok['Дана Қасымова'], p_id: R.id, p_answers: ['5', null, { x: 1 }, 99999, -5, 2.5, true, '<b>', [], 0], p_ms: 20000 })).my_ok === 0);
  const a4 = await rpc('esep_room_submit', { p_token: tok['Санжар'], p_id: R.id, p_answers: ok.map((v, i) => i < 8 ? v : 1), p_ms: 9999999 });
  T('8 of 10 at 40 s', a4.my_ok === 8 && a4.my_ms >= 40000 && a4.my_ms <= 45000, a4);
  s = await rpc('esep_room_state', { p_token: tok['Ерасыл'], p_id: R.id });
  T('state while others still play: no ranking, no scores of others, my facts are gone', s.status === 'running' && s.my_done && s.done_count === 4 && !('items' in s && s.items) && !('top' in s), s);
  ...
  await at(25); await rpc('esep_room_submit', { p_token: tok['Бөтен'], p_id: R.id, p_answers: ok.map((v, i) => i < 8 ? v : 1), p_ms: 25000 });
  ...
  T('a pupil sees the top three and her own place — not the names below', s.top.length === 3 && s.me.place === 5 && s.me.ok === 0 && s.me.beaten === 0 && s.me.stars === 1 && !JSON.stringify(s).includes('Санжар'), s);
  ...
  T('record: 1 played · 1 win · 1 podium · 4 beaten · ★3; the finished room is no longer «mine»', l.record.played === 1 && l.record.wins === 1 && l.record.podium === 1 && l.record.beaten === 4 && l.record.stars === 3 && l.mine === null && l.last.length === 1 && l.last[0].place === 1, l);
  T('after the end nothing more can be handed in; an outsider still sees nothing', (await rpc('esep_room_submit', { p_token: tok['Мадина'], p_id: R.id, p_answers: ok, p_ms: 1 })).error === 'not_found'
    && (await rpc('esep_room_state', { p_token: tok['Мадина'], p_id: R.id })).error === 'not_found');

  // ── the clock, ties, and a player who never hands in
  const R2 = await rpc('esep_room_create', { p_token: tok['Айгүл'], p_table: 0 });
  s = await rpc('esep_room_state', { p_token: tok['Айгүл'], p_id: R2.id });
  ...
  const mix = (await rpc('esep_room_state', { p_token: tok['Айгүл'], p_id: R2.id })).items;
  T('a mix: ten DIFFERENT facts, every one from a table the host has passed (2,3,5,10) against 2…9', mix.length === 10 && new Set(mix.map(([a, b]) => [a, b].sort().join('x'))).size === 10
    && mix.every(([a, b]) => ([2, 3, 5, 10].includes(a) && b >= 2 && b <= 9) || ([2, 3, 5, 10].includes(b) && a >= 2 && a <= 9)), mix);
  await pg.query(`update esep_private.rooms set started_at = now() - interval '60 seconds' where id=$1`, [R2.id]);
  await rpc('esep_room_submit', { p_token: tok['Айгүл'], p_id: R2.id, p_answers: right(mix), p_ms: 20000 }); await rpc('esep_room_submit', { p_token: tok['Ерасыл'], p_id: R2.id, p_answers: right(mix), p_ms: 20000 });
  ...
  await pg.query(`update esep_private.rooms set ends_at = now() - interval '11 seconds' where id=$1`, [R2.id]);   // the three minutes are over
  T('time is up: late answers are refused', (await rpc('esep_room_submit', { p_token: tok['Санжар'], p_id: R2.id, p_answers: right(mix), p_ms: 1000 })).error === 'closed');
  ...
  T('a teacher room is never «mine as host» — is_host is a plain false, not null', true);

  // ── lobbies lapse; limits
  const R3 = await rpc('esep_room_create', { p_token: tok['Ерасыл'], p_table: 2 });
  ...
  for (let i = 0; i < 9; i++) { const x = await rpc('esep_room_create', { p_token: tok['Ерасыл'], p_table: 2 }); await rpc('esep_room_leave', { p_token: tok['Ерасыл'], p_id: x.id }); }
  T('ten rooms a day', (await rpc('esep_room_create', { p_token: tok['Ерасыл'], p_table: 2 })).error === 'daily_limit');

  // ── one closed tab must not hold the podium hostage
  const R4 = await rpc('esep_room_create', { p_token: tok['Санжар'], p_table: 5 }); await rpc('esep_room_join', { p_token: tok['Мадина'], p_code: R4.code }); await rpc('esep_room_start', { p_token: tok['Санжар'], p_id: R4.id });
  const f4 = (await rpc('esep_room_state', { p_token: tok['Санжар'], p_id: R4.id })).items;
  ...
  await rpc('esep_room_submit', { p_token: tok['Санжар'], p_id: R4.id, p_answers: right(f4), p_ms: 1 });
  ...
  T('a pupil who has handed in is free to go elsewhere while the others still play', await (async () => {
    const x = await rpc('esep_room_create', { p_token: tok['Санжар'], p_table: 5 }); await rpc('esep_room_join', { p_token: tok['Мадина'], p_code: x.code }); await rpc('esep_room_join', { p_token: tok['Бөтен'], p_code: x.code });
    await rpc('esep_room_start', { p_token: tok['Санжар'], p_id: x.id }); const f = (await rpc('esep_room_state', { p_token: tok['Мадина'], p_id: x.id })).items;
    await pg.query(`update esep_private.rooms set started_at = now() - interval '30 seconds' where id=$1`, [x.id]); await rpc('esep_room_submit', { p_token: tok['Мадина'], p_id: x.id, p_answers: right(f), p_ms: 1 });
    const y = await rpc('esep_room_create', { p_token: tok['Мадина'], p_table: 5 }); const okk = y.id > 0; await rpc('esep_room_leave', { p_token: tok['Мадина'], p_id: y.id });
  ...
  const live = await rpc('esep_room_create', { p_token: tok['Санжар'], p_table: 5 });
  ...
  // ── the teacher
  T('teacher functions need the teacher token', await (async () => { try { await rpc('esep_t_room_create', { p_token: tok['Айгүл'], p_table: 7 }); return false; } catch (e) { return /teacher session required/.test(e.message); } })());
  const TR = await rpc('esep_t_room_create', { p_token: TT, p_table: 7, p_klass: ' 3а ' });
  T('the teacher opens a room on ANY table (7 — nobody here has passed it), listed for class 3А', /^\d{4}$/.test(TR.code) && (await rpc('esep_room_list', { p_token: tok['Дана Қасымова'] })).open.some(o => o.id === TR.id && o.by_teacher && o.table === 7)
    && !(await rpc('esep_room_list', { p_token: tok['Бөтен'] })).open.length, TR);
  ...
  await rpc('esep_t_room_start', { p_token: TT, p_id: TR.id }); const ti = (await rpc('esep_room_state', { p_token: tok['Дана Қасымова'], p_id: TR.id })).items;
  await pg.query(`update esep_private.rooms set started_at = now() - interval '50 seconds' where id=$1`, [TR.id]);
  await rpc('esep_room_submit', { p_token: tok['Дана Қасымова'], p_id: TR.id, p_answers: right(ti), p_ms: 15000 }); await rpc('esep_room_submit', { p_token: tok['Айгүл'], p_id: TR.id, p_answers: right(ti).map((v, i) => i ? v : 0), p_ms: 9000 });
  ...
  ts = await rpc('esep_t_room_state', { p_token: TT, p_id: TR.id });
  T('teacher closes the race early → the WHOLE ranking, including who did not finish', ts.status === 'done' && JSON.stringify(ts.ranking.map(k => [k.name, k.ok, k.place, k.finished])) === JSON.stringify([['Дана Қасымова', 10, 1, true], ['Айгүл', 9, 2, true], ['Бөтен', null, 3, false]]), ts.ranking);
  T('teacher list shows it', (await rpc('esep_t_rooms', { p_token: TT })).some(x => x.id === TR.id && x.status === 'done' && x.n === 3));
