# 数据库上锁 + 班级周榜 · 上线手册

**现状**：浏览器拿着公开的 key 直接读写 `students` / `events` 两张表——任何人都能导出全部孩子的姓名、班级、PIN 和答题记录，也能改、能删。教师 PIN `1234` 写在公开源码里。

**改完之后**：浏览器不再碰表，只调用十几个 `esep_*` 函数；每个函数在服务端校验会话 token（或教师密码），只动这个 token 有权动的那一行。PIN 在服务端比对，只存哈希。班级周榜由服务端汇总，浏览器只拿到「本班前五名的名字和分数、我的名次、本年级各班人均」。

分三步，前两步可回滚。**顺序不能换。**

---

## 第 0 步 · 备份 + 确认表结构（5 分钟）

1. Supabase Dashboard → Database → Backups，确认有今天的备份；或者在 Table editor 里把 `students`、`events` 各导出一份 CSV。
2. SQL editor 里跑：

```sql
select table_name, column_name, data_type from information_schema.columns
 where table_schema='public' and table_name in ('students','events') order by 1, ordinal_position;
select tablename, policyname, roles, cmd from pg_policies where schemaname='public';
select extname, extnamespace::regnamespace from pg_extension where extname='pgcrypto';
select tablename, tableowner from pg_tables where schemaname='public' and tablename in ('students','events');
select table_name from information_schema.views where table_schema='public';   -- 有没有别的 view 把这两张表露出去
```

迁移脚本假设（从 `core.js` / 教师页的读写推断，**我没有连过线上库**）：
`students(id, name, pin, klass, state jsonb, time_ms, last_seen, created_at)`、`events(student_id, t, ev jsonb)`。`id` 是 uuid 还是整数都行，脚本不假设。
**以下任一情况先停，把查询结果发给我：** 列名对不上；pgcrypto 不在 `extensions` schema；两张表的 owner 不是 `postgres`（那样第 3 步的 `enable row level security` 会失败）；`public` 里有引用这两张表的 view。
不要对这两张表用 `FORCE ROW LEVEL SECURITY`——函数是靠「表的 owner 不受 RLS 限制」通过的。

## 第 1 步 · `01_additive.sql`（只增不删，线上不受影响）

在 SQL editor 里整段运行。可重复运行。然后：

```sql
select count(*) from public.students where pin_hash is null;          -- 应为 0（PIN 不是 4 位数字的旧行会留下，单独看一下）
select esep_private.set_teacher_secret('这里换成一句长的话，至少 12 个字符');  -- 教师密码，只存哈希；以后改密码再跑一次这句
select esep_private.set_join_code('алма27');                              -- 学校口令（可选但强烈建议，见下）；传 '' 关闭

-- 冒烟测试：把四个函数都走一遍（类型不匹配只有在运行时才会暴露）
with l as (select public.esep_login('__smoke__', '0000', '9Z', 'алма27') v)
select v->>'token' is not null                                            as login_ok,
       public.esep_save(v->>'token', '{"x":1}'::jsonb, 5)                  as save_ok,
       public.esep_events(v->>'token', '[{"t":null,"ev":{"ev":"answer","ok":true,"u":"smoke"}}]'::jsonb) as events_1,
       public.esep_board(v->>'token') ? 'me'                               as board_ok
  from l;                                                                  -- 期望：t, t, 1, t
delete from public.students where name = '__smoke__';
```

**学校口令（join code）**：设了之后，**新名字**第一次注册必须输口令（老师在班里说一次），老学生永远不用输。不设的话，网上任何人都能注册进任意班、看到该班周榜前五名的名字、往教师列表里塞垃圾账号。

Dashboard → Settings → API → *Exposed schemas* 里**不要**加 `esep_private`（默认就没有）。

## 第 2 步 · 部署新客户端（分支 `db-lockdown-board`）

合并、等 GitHub Pages 发布（约 1–10 分钟）。然后亲自走一遍：

- 学生：用一个已有账号的名字 + PIN 登录 → 进任意路线答两题 → 回首页看到今日目标 +2；Table editor 里 `events` 有新行，`ev` 里带一个 `u` 字段。
- 教师：`teacher/` 用第 1 步设的密码登录 → 列表、单个学生、改站、重置 PIN 都能用。
- 输错 PIN → 提示「PIN басқа」。

**孩子们会感觉到的变化**：所有人要重新输一次 PIN（旧会话没有 token）；「这台设备以前登录过」的按钮现在只帮忙填名字，PIN 还是要输；忘了 PIN → 教师页里重置（哈希不可逆，看不到原 PIN）。

这一步之后新旧客户端都能用，**但新客户端注册的学生没有明文 PIN，旧缓存页面登不进他们的号**——所以验证完就接着做第 3 步，不要隔夜。

## 第 3 步 · `02_lock.sql`（上锁，可回滚）

运行后验证匿名 key 真的读不到了（把 URL 和 key 换成 `core.js` 里那两个）：

```bash
curl -s "https://<project>.supabase.co/rest/v1/students?select=name,pin&limit=1" -H "apikey: <publishable key>"
# 期望：permission denied / 401 / 空数组之外的任何「拿到数据」都算失败
```

还开着旧页面的设备会提示「Қосылу мүмкін болмады」，刷新即可（各页 `?v=9`）。

**回滚**：跑 `02_rollback.sql`，并把客户端回退到上锁前的提交。第 1 步的东西可以留着。

## 第 4 步 · 一周后 · `03_drop_plain_pin.sql`（不可逆）

确认不需要回滚了，再删明文 `pin` 列。先备份。

---

## 之后随时 · `04_challenges.sql`（同学挑战「жарыс»，只增不删，可重复跑）

第 1 步之后任何时候都能跑；没跑之前首页只是不显示挑战卡片，别的不受影响。规则全部在服务端：

- 异步：A 先打，B 下次打开 app 再打，不需要同时在线；同一组 10 道题由服务端生成并存下来；**答案由服务端判分**，浏览器不上报分数。
- B 打完之前，任何函数都不会返回 A 的成绩。
- 只能选**双方都已通过**的乘法表站（`AR-04/05/06/09/10/13–16` ↔ ×2,5,10,3,4,6–9；映射写在 `esep_private.ch_stage`，必须与 `ar/stages.js` 一致）——比赛是巩固已会的内容，不是赶新课。
- 只限同班；每人每天最多发起 3 场；同一对手有未完成的挑战时不能再发；7 天没人应战自动失效；`tester` 不参与。
- 赢 3 星、输 1 星、完全打平各 2 星；用时取浏览器上报值与服务端实际经过时间的较小者。

页面：`challenge/`（选人 → 选表 → 作答 → 结果），首页显示「收到的挑战」和「我发起的挑战出结果了」。

## 自己演练（不碰线上）

```bash
# 本地起一个一次性的 Postgres 16（trust 认证，端口 54329，socket 在 /tmp），然后：
npm i pg playwright
node supabase/test/run.js    # 56 项：迁移可重复跑（含第 3 步之后）、每个函数以 anon 角色测、限流、口令、11 万条事件下周榜 <10 ms、上锁后直接读写全部被拒、回滚
node supabase/test/run_challenges.js   # 21 项：挑战函数（资格、保密、服务端判分、平局、每日上限、过期）
node supabase/test/e2e_challenge.js   # 11 项：两个浏览器各扮一个学生，走完一整场
node supabase/test/e2e.js    # 22 项：真页面 → 拦截所有 supabase 请求 → 以 anon 角色打到本地库；含恶意 state / 事件、离线、教师页
node supabase/test/e2e_teacher_print.js   # 20 项：教师页关卡带名称（表格、CSV、学生卡）、整班打印报告（每条路线最高几关附 3 道例题、⚡ 速度关不出题、PV / 未知关卡只列编号、同种子同例题）
```

`test/00_baseline_guess.sql` 是我**猜的**现有表结构，只用于演练，绝不要在线上跑。

## 第 6 步（可选）· `05_rooms.sql` —— 挑战房间（жарыс бөлмесі）

**只增不删、可重复运行**，只需要 01 已经跑过（不再依赖 04）。没跑之前，首页和教师页只是不显示房间入口。跑过本脚本初稿（只有乘法表的那一版）的库可以直接再跑这一版：脚本会补列、关掉初稿留下的房间、删掉旧签名的函数。

是什么：一个学生（或老师在教师页）开房间，选**任何一个关卡**（AR / FR / TE / WP 四条路线，共 77 个；AR 的 4 个限时速算关本身就是游戏，不进房间）→ 得到 4 位房间号 → 全校任何人输房间号就能进（同班同学首页直接出现「某某开了房间 · Қосылу」）→ 开房间的人点「Бастау」→ 所有人**同一时刻**拿到**同一张卷子** → 排名。
PV（位值）是早期的单文件页面，没有独立的出题模块，进不了房间；把它改成规范路线之后自动就能用。

**题从哪来（关键设计）**：服务端不出题，只发一个**随机种子**。每个浏览器用路线自己的出题代码 + 这个种子生成卷子（`room/routes.js`：出题期间把 `Math.random` 换成种子发生器，并把 `Array.prototype.sort` 换成固定算法——不同浏览器引擎调用比较函数的顺序不同，不换的话 Chrome 和 Safari 会出不同的题）。好处：任何按 ROUTE_CONVENTION 写的关卡**零改动**就能进房间，题面、图、分数的显示都和练习时一模一样；学生之间从不传递题目或 HTML。`tests/room_sheets.js` 对全部 77 个关卡验证「同种子同卷子」（包括换一种排序实现）。

**服务端管什么**：谁能开什么（学生只能开自己 `passed` 的关卡，老师任意）、谁在房间里、何时开始结束、**用时（只认服务端的钟，刷新页面占不到便宜）**、排名（先答对数后用时）、学生只看前三名 + 自己名次且只看到名字第一个词、错 10 次房间号锁 10 分钟、每人同时一个房间/每天 10 个、30 分钟没开始自动关、开始后不能加入、最多 60 人、房主交卷后（20 秒以后）或老师可以提前结束；时间到后还有 30 秒宽限接收最后一刻交的卷（用时封顶为比赛时长）。

**服务端不管什么（要知道的取舍）**：答案对不对由浏览器用 `Core.isCorrect` 判——和练习、关卡测试、班级周榜完全同一套（分数、带单位的答案等的判法也因此和平时一致）。浏览器交上来的是每题「题面 / 标准答案 / 学生填的 / 对错」，服务端据此数分。会用浏览器开发者工具的孩子可以伪造——这和本应用其它地方的信任级别相同。对应的保障也相同：**老师看得到每个人每道题填了什么**——而且教师页上显示的题目和标准答案是教师页**自己用种子重新生成**的，学生的浏览器只贡献「填了什么、对或错」，按题号对上；哪台设备报上来的卷子指纹和教师页生成的不一样，会被标「⚠ басқа парақ」。全班错得最多的题也是在教师页按题号从对错标记里数出来的，学生没法往里塞文字。

每场题量和时间由关卡类型决定（`room/routes.js` 的 `rule`）：乘法表/除法口算 10 题 3 分钟；应用题（WP）5 题 8 分钟；其它 6 题 7 分钟。服务端把它们限制在 3–12 题、1–10 分钟。
星星：第 1 名 3 颗；第 2–3 名 2 颗（至少 4 人时）；其余交卷的 1 颗；没交卷 0 颗。战绩：场数、第一名次数、前三次数、累计超过多少人。

教师页「Жарыс бөлмесі」：选路线 + 关卡 + 班级 → 大号房间号（投屏）→ 实时看谁进来、谁交卷 → 结束后完整排名、每人的错题（题面按路线原样显示，含分数式；学生填的 → 标准答案）、**全班错得最多的题**。
**维护注意**：改了任何路线的 `stages.js` / `figs*.js` / `generate*.js`，要把 `room/routes.js` 里的 `V` 加一；否则两台缓存了不同版本的设备会从同一个种子生成不同的卷子（表现为「⚠ басқа парақ」）。
轮询：房间里每 2 秒问一次（`esep_room_state` 用只读的会话检查，不写库）；30 个孩子 ≈ 15 次请求/秒。

测试：`node tests/room_sheets.js`（77 个关卡的卷子，在真正独立的 JS 环境里验证同种子同卷子，13 项）、`node supabase/test/run_rooms.js`（数据库 57 项，全部以 anon 角色）、`node supabase/test/e2e_rooms.js`（三个学生 + 老师、四个浏览器：学生开分数关房间、老师开应用题房间；含「开始」回复迟到、出题文件第一次下载失败、比赛中途刷新，26 项）。上线前做过两轮独立的对抗式评审。
一对一挑战（04）目前仍只有乘法表；要放开可以照搬同一套「种子 + 路线出题」的做法。

## 之后随时 · `06_stars.sql`（班级榜改按星星排，只增不删、可重复跑）

第 1 步之后任何时候都能跑，**不建任何表、不加任何列**，只替换 `esep_board` 并加四个私有辅助函数。04 / 05 没装也能跑：没装的部分算 0 分，装完之后把本脚本再跑一遍就自动接上。

**为什么改**：原来榜上那个数字是「本周一次答对、没用提示的题数」—— 孩子在 app 里别的任何地方都看不到它，而且每周一清零。星星是他们本来就在收集、每个关卡上都看得见的东西。owner 2026-09-21 定的三条：

- 路线星（关卡测）+ 挑战星 + 房间星，**全算**；
- 排名按**累计总数**，旁边显示「本周 +N」；
- **诊断跳过的关卡不给星** —— 定位分出来的关卡和老师手动设的关卡都没有考试记录，自然是 0。星星只能靠考出来。

一颗星值多少，三处代码同一套规则：关卡测取历史最好一次，10/10 → 3、≥9/10 → 2、≥8/10 → 1；挑战赢 3 平 2 输 1；房间第 1 名 3、第 2–3 名 2（满 4 人时）、交卷 1。

路线星直接从 `students.state` 里现算（和孩子自己浏览器读的是同一份 jsonb），**不建缓存表**，所以没有会漂移的第二份数据，也不需要回填。「至今」和「本周之前」两个数由 `esep_private.route_stars2` 一次扫描同时算出来。演练里 300 个学生、每人一条 41 关的完整路线，一次 `esep_board` 用 **94 ms**（先前每关调一次函数的写法是 425 ms）。

**回滚**：把 `01_additive.sql` 里 `esep_board` 那一段重新跑一遍就回到旧口径，没有表或列需要清理。

客户端要一起上：门户顶部的星星总数、房间卡片上的生涯星星、`pv/bridge.js` 把过关成绩写进 `tests`（不改这个，只做 PV 的孩子永远 0 星）。

演练两套：

```bash
node supabase/test/run_stars.js    # 37 项：星星算法、脏数据、旧格式 state、周增量、诊断关卡不计分、挑战/房间计入、tester 排除、anon 权限、300 人性能
node supabase/test/e2e_stars.js    # 14 项：端到端 —— 真的 core/core.js、真的 pv/bridge.js、真的门户脚本，打到装好 01+04+05+06 的本地库
node supabase/test/e2e_tester.js  # 25 项：tester 账号 —— 五条路线各自按自己 index.html 的顺序真的启动一遍，看 129 个关卡是不是真开着
node supabase/test/e2e_diag.js    # 19 项：诊断定位 —— 真 DOM（jsdom）里把整套诊断打完；核心是「只会 1..K 站的孩子必须落在第 K+1 站」，不需要数据库
node tests/cachebust.js           # 3 项：所有页面对同一个 core/ 文件的 ?v 必须一致（不需要数据库，秒级）
node tests/pv.js                  # 4 项：PV 老应用端到端 —— jsdom 里跑它自己的脚本，开关卡、答题、按「Келесі」
```

只读排查：`supabase/q_ar_misplaced.sql` —— 2026-09-22 之前被诊断上限放低的孩子是谁。一条 SELECT，什么都不改；
认的是「答过、但 `diag.results` 里没有裁定」的那个站。`дұрыс = 2` 的孩子请他们按一次「Қайта диагностика」即可，
数据库里不需要动任何东西。

`e2e_stars.js` 里我们自己的代码一行都没有替身：假的只有浏览器（一个小 DOM，`<script>` 标签会真去加载那个文件）和 PV 老应用自己的那几个函数（bridge.js 包的就是它们）。它验证的是三块能对上 —— core.js 存下的关卡测、bridge 写进去的 PV 成绩、服务端判的房间名次，最后门户把三者合成同一个星星数。

## 之后随时 · `07_practice_stars.sql`（练习也给星星，只增不删、可重复跑）

需要 01 和 06。owner 2026-09-21 定的两条：

- **无提示、一次答对满 5 道 → 1 颗**（累计，不按天取整：今天 4 道、明天 1 道照样给）
- **完成当天目标（15 道题）→ 1 颗**
- 关卡测不变：10/10 三颗、9/10 两颗、8/10 一颗

为什么是这个比例：最早那版 PV 是「一题一颗」（`pv/index.html:2366` 到今天还在跑，只是没人读它）。照那个比例一周能拿 80 颗，而全平台 129 个关卡全过也才 387 颗 —— 练习星会把别的全淹掉。现在这个比例一周约 15 颗，两边大致平衡。

数据来源是 `esep_private.day_stats`，服务端每次收事件时本来就在写，一人一天一行。**不需要客户端记任何东西，也没有会漂移的第二份**；周增量因为行带日期所以是精确的。那张表里的 `score` 本来就是「一次答对、没用提示、不在诊断里」（`esep_private.scores`），正好就是练习星的条件。

**唯一缺的是「每天答了多少题」**：`day_stats` 只有 `events`（所有事件，含提示和重试）和 `score`（完美的那些），而首页那个圆环数的是答题数。所以本脚本给 `day_stats` 加了一列 `answers`，从事件表回填，并让 `esep_events` 以后一直记。

> ⚠️ **本脚本会替换 01 里的 `public.esep_events`** —— 函数体就是 01 的，只多了一个计数器。以后要是改了 01 里那个函数，记得把本文件再跑一遍。

一个刻意的取舍：周日剩下的余数（比如攒了 3 道）在周一凑满 5 道时给星，这颗星算在本周。反过来把余数抹掉会让孩子因为周界在哪而吃亏。

回滚：把 01 的 `esep_events` 和 06 的 `esep_board` 两段重新跑一遍即可；`answers` 列留着不碍事。

演练：`node supabase/test/run_practice_stars.js`（17 项：两种比例、余数跨天跨周、目标数的是答题数而不是完美题、tester 排除、anon 权限、`esep_events` 仍然正常且开始记 `answers`）。

## 之后随时 · `08_classes.sql`（班级只能选，不能打字，只增不删、可重复跑）

需要 01。

**为什么**：班级本来是一个自由输入框。同一个班，一个孩子打「5」、一个打「5 БАРЫС」、一个打「БАРЫС5」、一个打「5БАРЫС」——服务端只做 `upper(btrim())`，于是这是**四个班**。结果班级榜（本班前五名 + 我的名次）对几乎所有人都悄悄失效了：每个孩子在一个只有自己的班里排名；班级对比要求一个班至少 3 人，所以谁都看不到；班级留空的孩子（真有）则永远没有榜。

**改成什么**：老师维护一份班级名单，孩子只能从里面选，打不了字。名单在本脚本里**从现有数据播种**，所以什么都不会丢、谁都不用重新注册；老师再在教师页上把重复的拼法合并掉——合并会把学生一起搬过去。

教师页多了一张「Сыныптар」卡片：每个班多少人、合并、新建、删空班；不在名单里的拼法会红字提示。另外 `esep_t_set_class` 可以把某个孩子放进某个班（给那些在本脚本之前就注册、或者选错了的）。

> ⚠️ **本脚本会替换 01 里的 `public.esep_login`** —— 只加了一条检查：班级必须是名单里的（空仍然允许）。不加这条的话，缓存着旧页面的设备还能写进脏值。
> 函数体是**用脚本从 01 里原样抄出来再插入那 3 行**的，不是手打的——手打那一版曾经悄悄弄丢了明文 PIN 兼容、名字里 `%`/`_` 的转义、重名循环和入学码限流四件事。`run_classes.js` 的第一项就是逐行比对这两份函数体，专门防这个。

回滚：把 01 的 `esep_login` 那一段重新跑一遍即可；`esep_private.classes` 表留着不碍事。

演练：`node supabase/test/run_classes.js`（27 项：播种、只接受名单内的班、合并/新建/删除、老师改某个孩子的班、以及 01 登录逻辑的全部原有行为——明文 PIN、两种限流、入学码——都还在）。

## 之后随时 · `09_my_stars.sql`（自己的星星总数，不需要班级）

需要 01 + 06 + 07。只加一个函数 `public.esep_stars(token)`，返回 `{n, week, parts:{route, practice, goal, challenge, room}}`。

**修的是什么**：主页顶上那个星星钱包本来是从班级榜取数的，而榜的第一条规则是「没有班级就没有榜」—— 它要跟同班同学比，没班级就没人可比。结果**没有班级的孩子钱包永远是 0**，哪怕下面房间卡片明明写着 ★ 11。2026-09-21 在线上抓到：走了 31 个站、当天目标达成、5 场比赛赢 3 场、房间星 11，头上写着 0。

自己的总数不是排名，不该依赖排名。`parts` 拆成五项，孩子问「我的星星哪来的」时能答，出了争议也不用开数据库。

演练：`run_practice_stars.js` 多了 4 项（含「没有班级也能拿到总数」），`e2e_stars.js` 多了 2 项——用真页面验证没班级的孩子钱包显示正确、同时榜卡片仍然正确地不出现。

## 之后随时 · `10_my_class.sql`（没班级的孩子自己选一次）

需要 01 + 08。只加一个函数 `public.esep_my_class(token, klass)`。

**为什么**：2026-09-21 合并完拼法之后，班级「5」有 34 人、510 颗星 —— 同时**还有 16 个孩子根本没有班级**。没班级就没有榜（`esep_board` 没人可比），页面上那块就是一片空白，从孩子的角度看就是榜消失了。

老师可以一个个放（`esep_t_set_class`），但 16 个孩子就是 16 次点击，而这件事他们自己都知道。所以：**班级为空的孩子可以自己选一次**。

「一次」就是这里全部的安全模型 —— 已经有班级的孩子改不了，否则谁都可以跳到自己能当第一的班里去，榜就没有意义了。**换班仍然只有老师能做**，因为那是看得见的操作。

顺带：「已经有班级」这个判断放在「班级是否存在」**之前**，所以拒绝的理由不会随目标班级变化 —— 不然那句拒绝本身就会告诉孩子哪些班存在。

客户端：门户在孩子没有班级时显示一张「Сыныбың көрсетілмеген」卡片，里面就是那个下拉。

演练：`run_classes.js` 多了 5 项。

## 之后随时 · `11_backfill_days.sql`（把孩子早就挣到的练习星补上，可重复跑）

需要 01 + 07。**不建表不加列**，只是把 `esep_private.day_stats` 按 `public.events` 重算一遍。

**为什么**：三种星能回溯到的距离不一样，只有一种是完整的。

| | 回溯到哪 |
|---|---|
| 关卡星 | **全部历史** —— 它们在学生自己的 `state.stages[].tests` 里，从 runner 存在的第一天起就在记 |
| 每日任务星 | **全部历史** —— 07 加 `answers` 列时就是从整张事件表填的 |
| 练习星 | **只有 15 天** —— 它数的是 `day_stats.score`，而 01 安装时只回填了 15 天，更早的还躺在 `events` 里没算 |

本脚本补的就是第三行。规则跟线上完全一致（01 的 `scores()`：一次答对、没用提示、不在诊断里、且不是用掉免费重试那次；07 的答题数口径）。**是重算不是累加**，跑两遍第二遍什么都不变；没有事件的日子不碰，所以星星只会涨不会掉。

跑完想看变化：

```sql
select count(*) as days, sum(score) as perfect_answers, sum(score)/5 as practice_stars_total
  from esep_private.day_stats;
```

演练：`run_practice_stars.js` 多了 3 项（90 天前的旧事件被数进来、带提示和重试过的仍然不算分、重复跑幂等）。

## 之后随时 · `13_grades.sql`（班级带上年级，只增不删、可重复跑）

需要 08。**这一条是必须的，不是装饰**：学校二年级有 Samuryq 和 Qyran，三年级也有 Samuryq 和 Qyran。
`esep_private.classes` 的主键只有名字，所以「SAMURYQ」一个名字就是**一行**——两届孩子会挤进同一块班级榜、
教师页也会显示成一个五十来人的班。

规矩就一条，脚本用约束把它钉死：

> **班名以自己的年级开头。** `2 SAMURYQ`　`3 QYRAN`　`3Ә`

这本来就是周榜的算法：`esep_board` 用 `substring(klass from '^\d+')` 取年级，去找**同年级的其它班**做人均分对比。
名字前面没有数字的班，榜上就没有可比的邻居。

脚本做四件事：

1. `classes` 加 `grade` 列，按现有名字回填；加约束，让列和名字前缀**永远不可能对不上**（名字才是真相——`students.klass` 存的是名字的副本）。原来没数字的老班级 `grade` 留空，行为跟以前完全一样。
2. `esep_t_class_add` 多一个年级参数：老师在一个框里打 `Samuryq`、在另一个框里选 `2`，存进去就是 `2 SAMURYQ`。名字本来就带数字（`3Ә`）照旧能用；两处数字打架就报错，不替老师猜。
3. `esep_t_classes` 把年级一起报出来，教师页按年级分组显示。
4. 建好这学校的四个班：`2 SAMURYQ` `2 QYRAN` `3 SAMURYQ` `3 QYRAN`。

**给已有的班补年级，不要改名**——孩子身上的 `students.klass` 还是老名字。正确做法是在教师页
**新建**带年级的班，再把老班**「біріктіру →」**合并过去：`esep_t_class_merge` 会把每个孩子一起搬过去，
然后删掉老的那一行。这就是合并这个功能存在的理由。

脚本末尾两条 `select` 是自检：第一条应当**0 行**，第二条应当正好列出上面那四个班。

前端跟着改了两处：登录卡和首页的班级下拉框按年级分组（`Core.klassOptions`，只有一个年级时不加分组标题），
教师页的「Сыныптар」卡按年级分段，新建班多一个年级下拉框。

演练：`run_classes.js` 从 35 项变 **50 项**（含「两个 Samuryq 是两个班」「列和名字对不上会被拒」
「九个字符的班名能登录」「她的榜只跟同年级的另一个班比，不跟三年级比」）；`tests/klass_options.js` 13 项。

## 周榜规则（`esep_board`）

- 分数 = 本周一（UTC+5）以来，**一次答对、没用提示、不在定位测里**的题数。重试后答对的不算（runner 会先记一条 `attempt`）。
- 计分发生在 `esep_events` 写入的那一刻，累加到 `esep_private.day_stats`（每人每天一行）；`esep_board` 只读这张小表，事件表再大也是毫秒级。01 脚本末尾会把最近 15 天的旧事件回填一次。
- 每个孩子每天最多收 4000 条事件，超出丢弃（防刷、防把库撑爆）。
- 只返回：本班前 5（名字 + 分数）、我的名次和分数、「比上周同一时刻多多少」、同年级各班的人均分（班里近 30 天活跃 ≥3 人才显示）。`tester` 账号不参与。
- 分数来自客户端上报的事件，会改浏览器的孩子可以刷分——试点阶段可以接受；PV 连点「Тексеру」的 bug 也会刷高，要在 PV 那边修。

## 顺手修掉的（都在 `core/core.js` / `teacher/index.html`）

- 离线进度丢失：登录时服务器旧状态会覆盖本地新状态 → 现在本地缓存**只有在含未发送的改动（`cache.dirty`，持久化）且比服务器新**时才胜出；已同步过的缓存永远不胜出，所以时钟偏快的设备不会一直压过别的设备。共用设备上，上一个孩子没发出去的进度会被单独存起来，等他下次在这台设备登录时再合并。
- 事件丢失：离开页面时发送后立刻清队列 → 现在队列只在正常同步确认后才清，事件带随机 `u`，服务端对 (学生, u) 去重，重发无害。
- 共用设备：上一个孩子没发完的事件不会再记到下一个孩子名下。
- 教师页：所有来自数据库的值都转义（学生能写这些行）；`esc()` 补上了引号。
- 登录：名字精确匹配（不分大小写），`*` `%` `_` 不再是通配符。错 PIN 按「名字 + 来源地址」计：同一地址 10 分钟错 8 次，只锁这个地址——外人没法把孩子锁在外面；同一名字来自所有地址合计 40 次才全局锁（限制分布式猜 PIN）。同一教室共用一个出口 IP，所以同学之间仍可能互相锁 10 分钟；教师重置 PIN 会清掉锁。
- 教师登录只按来源地址限流，没有全局锁。教师页有了「шығу」，退出即作废 token；token 12 小时过期。
- 教师页把来自学生 state 的每个数字强制转成数字、丢弃非对象的站点、单行渲染失败不拖垮整页；CSV 导出对以 `= + - @` 开头的单元格加 `'`；手动设站前先取该生最新 state 并盖 `_t` 时间戳。

## 没做 / 已知限制

- 教师只有**一个共用密码**，没有分老师、分班的账号。要细分得上 Supabase Auth。
- 来源地址取自请求头 `cf-connecting-ip` / `x-forwarded-for`（PostgREST 的 `request.headers`）。**我没法在本地验证 Supabase 实际转发哪个头**；如果都取不到，限流退化为「只按名字」，功能不受影响。上线后可以在 SQL editor 里看 `select name_key from esep_private.login_fails order by t desc limit 5;`，`|` 后面有地址就说明取到了。
- 周榜分数来自客户端上报，懂技术的孩子可以伪造事件刷分（每天 4000 条封顶）。
- `esep_save` 仍是整块覆盖（后写赢）。同一孩子两台设备同时做题仍会互相覆盖。
- 4 位 PIN 的哈希离线可以秒破——哈希只是不让人「顺手看到」，真正的保护是匿名角色根本读不到这一列。
- `core/core-stub.js`（路线作者本地用的假 core）没有加 `days()` / `board()`，路线用不到它们。
